...
describe('Likes testing suite', () => {
  ...
  it('Like should be sent correctly', async () => {
    const result = await likesController.sendLike({
      userId: 4,
      advertisementId: advertisement.result!.id,
    });
    const like = Like.findOne({
      where: {
        user_id: 4,
        advertisement_id: advertisement.result!.id,
      },
    });
  ...
  it('Available likes should be reduced according to sent likes', async () => {
    ...
    expect(userStats!.available_likes_to_send).toBe(9);
  });
  ...
  it('Available likes should be refunded by +1 when like dismissed', async () => {
    ...
    expect(userStatsBeforeDismiss!.available_likes_to_send).toBe(9);
    expect(userStatsAfterDismiss!.available_likes_to_send).toBe(10);
  });
  ...
  it('Advertisement owner should be able to match with user who liked', async () => {
    ...
    expect(userStatsOfOwner!.active_chats).toBeGreaterThan(
      userStatsOfOwnerBefore!.active_chats,
    );
    expect(userStatsOfLiker!.active_chats).toBeGreaterThan(
      userStatsOfLikerBefore!.active_chats,
    );
