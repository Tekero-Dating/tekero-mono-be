...
import {
  ADS_MSG_PATTERNS,
  IAdsController, IArchiveAdv, ICreateAdv, IEditAdv, IPublishAdv
...
export class AdsController implements IAdsController{
  ...
  async createAdv(
    ...
    try {
      ...
      this.logger.log('createAdv Request processed successfully', { userId: payload.userId });
      return {
        success: true,
        result: adv
      }
  ...
  async editAdv(@Payload() payload: IEditAdv.Request, @Ctx() context: RmqContext): Promise<IEditAdv.Response> {
    ...
    try {
      const adv = await this.addService.editAd(payload.userId, payload.advId, payload.fields);
      this.logger.log('editAdv Request processed successfully', { userId: payload.userId });
      return {
        success: true,
        result: adv
      }
  ...
  async publishAdv(@Payload() data: IPublishAdv.Request, @Ctx() context: RmqContext): Promise<IPublishAdv.Response> {
