...
import { ChatTypesEnum, MessageTypesEnum } from '../../contracts/db/models/enums';
...
export class ChatService {
  ...
  private async checkIfUserAllowedInChat(userId: number, chatId: number): Promise<boolean> {
    this.logger.log(`Private func checkIfUserAllowedInChat`, { userId, chatId });
  ...
  async sendMessage(userId: number, chatId: number, content: string): Promise<Message> {
    ...
    try {
      ...
      this.logger.log('sendMessage: Message created and sent', { userId, chatId });
      return message;
    } catch (error) {
      this.logger.error('sendMessage: something critical happen while creating message', { error, userId, chatId });
      throw new InternalServerErrorException('Can not send message at this time ');
    }
  }

  async getChatHistory(userId: number, chatId: number): Promise<(Message & { user: { firstName: string } })[]> {
    this.logger.log('getChatHistory: ', { userId, chatId });
    const validChat = await this.checkIfChatTypeValid(chatId);
    this.logger.log('getChatHistory: valid chat?', { userId, chatId, validChat });
    if (!validChat) {
    ...
    const isUserInChat = await this.checkIfUserAllowedInChat(userId, chatId);
    this.logger.log('getChatHistory: allowed in chat? ', { userId, chatId, isUserInChat });
    if (!isUserInChat) {
      this.logger.warn('getChatHistory: not allowed', { userId, chatId });
      throw new NotFoundException('User is not allowed in this chat.');
    }

    this.logger.log('getChatHistory: getting all messages for the given chat', { userId, chatId });
    try {
      ...
      this.logger.error('getChatHistory: Something critical happen on the server: ', { error, userId, chatId });
  ...
  async uploadImage({userId, chatId, imageId }) {
    ...
    try {
      ...
      this.logger.error('uploadImage: something happen on the server side', { error, context });
