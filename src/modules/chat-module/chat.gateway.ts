...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleJoinChat(@ConnectedSocket() client: Socket): Promise<void> {
    ...
    await rmqSend<any, any>(
      ...
      ({ success, result, error }) => {
        if (success) {
          this.logger.log('Join request: got chat history successfully.', {
            ...context,
          });
          ...
          this.logger.log(
            `Join request: Client ${client.id} joined chat room: ${client.data.chatId}`,
            { ...context },
          );
        } else {
  ...
  async handleSendMessage(
    ...
    await rmqSend<ISendMessage.Request, ISendMessage.Response>(
      ...
      ({ success, result, error }) => {
        if (success) {
          ...
          this.logger.error('Send message: Message can not be sent', {
            error,
            ...context,
          });
          const { message: errorMessage } = TekeroError(error);
    ...
    this.logger.log(
      `Message sent to room ${client.data.chatId}: ${client.data.user.sub}: ${message}`,
    );
  }
  ...
  async handleUploadImage(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    payload: { image: string; fileName: string; mimeType: string },
  ) {
    try {
