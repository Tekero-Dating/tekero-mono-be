...
export class ChatController implements IChatController {
  ...
  async getChatHistory(@Payload() payload, @Ctx() context: RmqContext) {
    try {
      const result = await this.chatService.getChatHistory(payload.userId, payload.chatId);
      return {
        success: true,
        result
      };
  ...
  async sendMessage(@Payload() payload, @Ctx() context: RmqContext) {
    try {
      const result = await this.chatService.sendMessage(payload.userId, payload.chatId, payload.message);
      return {
        success: true,
        result
      };
  ...
  async sendMedia(@Payload() payload, @Ctx() context: RmqContext) {
    try {
      const result = await this.chatService.uploadImage({ userId: payload.userId, chatId: payload.chatId, imageId: payload.imageId })
      return {
        success: true,
