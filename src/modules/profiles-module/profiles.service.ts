...
export class ProfilesService {
  ...
  async updateUserProfile(
  ...
  ): Promise<(UserProfile & { profile_owner: User }) | null> {
    const userProfileValues = {
      ...
      ...(values?.gender_expression && {
        gender_expression: values.gender_expression,
      }),
    ...
    const userValues: Partial<User> = {
      ...
      ...(values?.profile_picture && {
        profile_pic_id: values.profile_picture,
      }),
    };
    ...
    return await Promise.all([
      ...
      await this.userRepository.update<User>(userValues, { where: { id } }),
    ]).then(async (result) => {
