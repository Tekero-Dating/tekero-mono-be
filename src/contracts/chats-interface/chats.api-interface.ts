...
export interface IChatController {
   ...
   */
  sendMessage(payload: ISendMessage.Request, context: RmqContext): Promise<ISendMessage.Response>;

  /**
   * Uploads media, sends to a client a mediaId so client know which image to display.
   */
  sendMedia(payload: ISendMedia.Request, context: RmqContext): Promise<ISendMedia.Response>;

  /**
   * Checks if user allowed in chat, chat existence, then sends all messages
   * from the chat history.
   */
  getChatHistory(payload: IGetChatHistory.Request, context: RmqContext): Promise<IGetChatHistory.Response>;
}
